...
def move_segment_endpoint():
    ...
    텍스트 이동 시 텍스트가 있던 슬라이드와 이동된 슬라이드의 요약 재생성
    """
    if request.method == 'OPTIONS':
        return jsonify({"status": "ok"}), 200
    ...
    try:
        ...
        if start_slide_key not in result_data or \
           "Segments" not in result_data[start_slide_key] or \
           start_segment_key not in result_data[start_slide_key]["Segments"]:
        ...
        updated_start_text = " ".join(current_text.replace(text_to_move, "").strip().split())
        ...
        try:
            ...
            from src.image_captioning import load_captioning_json

            captioning_path = os.path.join(job_dir, 'captioning_results.json')
            if os.path.exists(captioning_path):
                with open(captioning_path, 'r', encoding='utf-8') as f:
                    captioning_data = json.load(f)
                # 요약 재생성
                for slide_id in [start_slide, target_slide]:
                    if slide_id == 0:
                        continue
                    slide_key = f"slide{slide_id}"
                    if slide_key in result_data:
                        segments = result_data[slide_key].get("Segments", {})
                        all_text = " ".join([seg.get("text", "") for seg in segments.values()]).strip()
                        summary = generate_summary(all_text, captioning_data, slide_id)
                        result_data[slide_key].update(summary)
                        print(f"슬라이드 {slide_id} 요약 갱신 완료")
        except Exception as summary_error:
            print(f"[요약 오류] {summary_error}")

        with open(result_path, 'w', encoding='utf-8') as f:
            json.dump(result_data, f, ensure_ascii=False, indent=2)

        if os.path.exists(result_path):
            print(f"result.json 저장 완료, 파일 크기: {os.path.getsize(result_path)} bytes")
